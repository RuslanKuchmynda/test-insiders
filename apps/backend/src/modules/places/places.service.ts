...
export class PlacesService {
  ...
  private async checkTripPermission(userId: string, tripId: string) {
    const trips = await db.select().from(tripSchema).where(eq(tripSchema.id, tripId));
    const trip = trips[0];
    ...
    const collaborators = await db
      ...
      .where(and(eq(tripCollaboratorsSchema.tripId, tripId), eq(tripCollaboratorsSchema.userId, userId)));
